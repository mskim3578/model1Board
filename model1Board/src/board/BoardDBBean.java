...
public class BoardDBBean {
	...
	public void insertArticle(BoardDataBean article, String boardid) throws Exception {
		...
		try {	pstmt = conn.prepareStatement("select nvl(max(num),0) from board");
	...
	public BoardDataBean getArticle(int num, String boardid) throws Exception {
		...
		try {
			...
			pstmt.executeUpdate();
			pstmt = conn
					.prepareStatement("select * from board where num = ? and boardid = ?");
			pstmt.setInt(1, num);
			pstmt.setString(2, boardid);
			rs = pstmt.executeQuery();
			if (rs.next()) {
...
public int updateArticle(BoardDataBean article, String boardid)
	...
	.prepareStatement("select passwd from board where num = ?");
			...
			if (rs.next()) {dbpasswd = rs.getString("passwd");
				if (dbpasswd.equals(passwd)) {	pstmt = conn
	.prepareStatement("delete from board where num=?");
					pstmt.setInt(1, num);
					pstmt.executeUpdate();
					x = 1;	} else x = 0; // 비밀번호 틀림
			}
		} catch (Exception ex) {
			ex.printStackTrace();
		} finally {	if (rs != null)
